...
import { getChromeUXReport } from "../../../services/seo/technical-seo-merged.service.js";
...
export async function generateCompleteSeoIntelligence({ chatId, userId, websiteUrl, chat }) {
  ...
  try {
    ...
    modules.crawl = { status: 'SUCCESS', websiteData, identity, researchData };
  } catch (e) {
  ...
  // == MODULE 2: Technical SEO (Evidence-based) ==
  ...
  try {
    ...
    if (chatId) {
      const evidenceReq = await getLatestEvidenceSnapshot(chatId);
      if (evidenceReq.success && evidenceReq.snapshot) {
        techResult = evidenceReq.snapshot.technicalSeoEvidence || null;
      }
    }
    modules.technicalSeo = { status: techResult ? 'SUCCESS' : 'UNAVAILABLE', data: techResult };
  } catch (e) {
    warnings.push({ code: 'TECHNICAL_SEO_FAILED', message: `Technical SEO load failed: ${e.message}` });
  ...
  try {
    ...
    if (chatId) {
      const evidenceReq = await getLatestEvidenceSnapshot(chatId);
      if (evidenceReq.success && evidenceReq.snapshot) {
        psResult = evidenceReq.snapshot.technicalSeoEvidence?.pageSpeed || null;
      }
    }
    
    const mobileOk = psResult?.mobile;
    const desktopOk = psResult?.desktop;
    modules.pageSpeed = {
      status: mobileOk || desktopOk ? 'SUCCESS' : 'UNAVAILABLE',
      mobile: mobileOk ? psResult.mobile : null,
      desktop: desktopOk ? psResult.desktop : null
    };
    console.log('[SEO MODULE RESULT]', { module: 'PageSpeed', status: modules.pageSpeed.status, mobile: !!mobileOk, desktop: !!desktopOk });
  } catch (e) {
    warnings.push({ code: 'PAGESPEED_FAILED', message: `PageSpeed load failed: ${e.message}` });
