...
import { collectEvidence, upsertEvidenceSnapshot } from "./evidence.service.js";
...
export const collectEvidenceHandler = async (req, res) => {
  ...
  try {
    ...
    // Store the snapshot (single canonical row per chat+user+websiteUrl — merges, never overwrites)
    const snapshot = await upsertEvidenceSnapshot({
      ...
      contentEvidence: {
        ...
        keywords: result.evidence?.keywords || [],
      },
