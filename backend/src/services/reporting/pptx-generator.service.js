...
export async function generatePptx(data) {
  ...
  try {
    ...
    const metrics = [
      { label: 'Overall Score', value: `${scores?.overallGrowthScore || 'N/A'}/100`, color: COLORS.primary },
      { label: 'TAM', value: safeStr(market?.tam), color: COLORS.secondary },
      { label: 'Direct Competitors', value: String((competitor?.direct || []).length || 0), color: COLORS.accent },
      { label: 'Technologies', value: String((technology?.technologies || []).length || 0), color: COLORS.danger }
    ];
    metrics.forEach((m, i) => {
      const x = 0.5 + i * 2.4;
      addMetricBox(pptx, m.label, m.value, m.color, x);
    });

    // Slide 4: Company Overview
    addSectionSlide(pptx, 'Company Overview', `${name} — Business Profile`, COLORS.primary);
    const companyFields = [
      ['Industry', safeStr(company?.industry)], ['Business Model', safeStr(company?.businessModel)],
      ['B2B / B2C', safeStr(company?.b2bOrB2C)], ['Target Market', safeStr(company?.targetMarket)],
      ['Headquarters', safeStr(company?.headquarters)], ['Employees', safeStr(company?.employeeEstimate)]
    ];
    ...
    pptx.addText(`TAM: ${safeStr(market?.tam)}`, { x: 0.5, y: 1.5, w: 9, h: 0.5, fontSize: 16, fontFace: 'Calibri', color: '333333' });
    pptx.addText(`SAM: ${safeStr(market?.sam)}`, { x: 0.5, y: 2.1, w: 9, h: 0.5, fontSize: 16, fontFace: 'Calibri', color: '333333' });
    pptx.addText(`SOM: ${safeStr(market?.som)}`, { x: 0.5, y: 2.7, w: 9, h: 0.5, fontSize: 16, fontFace: 'Calibri', color: '333333' });
    if (market?.growthRate && market.growthRate !== 'Unknown') {
      pptx.addText(`Growth Rate: ${safeStr(market.growthRate)}`, { x: 0.5, y: 3.3, w: 9, h: 0.5, fontSize: 16, fontFace: 'Calibri', color: '333333' });
    }
    ...
    // Slide 9: Strategic Roadmap (Action Plan)
    ...
    const hasAnyTask = planPhases.some(p => p.task !== null && p.task !== '');
    ...
    if (!hasAnyTask) {
      pptx.addText('Connect analytics/ad account to generate verified action items.', { x: 0.5, y: 5.5, w: 9, h: 0.5, fontSize: 12, fontFace: 'Calibri', color: COLORS.muted, italic: true });
    }
...
function getFirstTask(tasks) {
  ...
  const t = tasks[0];
  const text = t.title || t.task || t.action || null;
  return text || null;
}
