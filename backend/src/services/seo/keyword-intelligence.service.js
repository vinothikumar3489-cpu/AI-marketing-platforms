...
export async function generateKeywordIntelligence({
  ...
  try {
    ...
    // Step 4: AI fallback — never return zero keywords
    const totalAfterEnrich = (result.primaryKeywords?.length || 0) +
      (result.secondaryKeywords?.length || 0) +
      (result.longTailKeywords?.length || 0) +
      (result.questionKeywords?.length || 0);

    if (totalAfterEnrich === 0) {
      console.log('🤖 [Keyword Intelligence] Zero keywords from all sources, generating AI fallback keywords...');
      const aiFallback = await generateAIFallbackKeywords(safeProductName, safeCompanyName, safeIndustry);
      if (aiFallback) {
        result.primaryKeywords = markArrayDataQuality(
          aiFallback.primary || [],
          DataSource.AI_INFERRED,
          ConfidenceLevel.LOW,
          { method: 'ai_fallback', reason: 'no_live_api_data' }
        );
        result.secondaryKeywords = markArrayDataQuality(
          aiFallback.secondary || [],
          DataSource.AI_INFERRED,
          ConfidenceLevel.LOW,
          { method: 'ai_fallback', reason: 'no_live_api_data' }
        );
        result.longTailKeywords = markArrayDataQuality(
          aiFallback.longTail || [],
          DataSource.AI_INFERRED,
          ConfidenceLevel.LOW,
          { method: 'ai_fallback', reason: 'no_live_api_data' }
        );
        result.questionKeywords = markArrayDataQuality(
          aiFallback.questions || [],
          DataSource.AI_INFERRED,
          ConfidenceLevel.LOW,
          { method: 'ai_fallback', reason: 'no_live_api_data' }
        );
        result.metadata = {
          ...result.metadata,
          totalKeywords: (result.primaryKeywords?.length || 0) +
            (result.secondaryKeywords?.length || 0) +
            (result.longTailKeywords?.length || 0),
          source: DataSource.AI_INFERRED,
          message: 'Keywords generated by AI fallback — no live API data available',
          qualityScore: calculateQualityScore([
            ...result.primaryKeywords,
            ...result.secondaryKeywords,
            ...result.longTailKeywords,
            ...result.questionKeywords
          ])
        };
      }
    }
...
async function generateAIFallbackKeywords(productName, companyName, industry) {
  const GROQ_API_KEY = process.env.GROQ_API_KEY;
  if (!GROQ_API_KEY) {
    return generateFallbackKeywords(productName, companyName, industry);
  }
  try {
    const prompt = `You are an SEO keyword research expert. Generate relevant SEO keyword ideas for a business.

Product/Company: ${productName || companyName || 'Unknown'}
...
Rules:
- 5-8 primary keywords (relevant, moderate-high volume terms)
- 5-10 secondary keywords (related terms, longer phrases)
- 5-10 long-tail keywords (specific, lower competition phrases)
- 3-5 questions (how/what/why questions customers ask)
...
- Prioritize specific, actionable, searchable phrases`;
    const response = await fetchGroqCompletion({
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an SEO keyword research specialist. Return valid JSON only.' },
    ...
    if (response.ok) {
      ...
      const toKeywordObj = (kw, source) => {
        ...
        return {
          ...
          intent: kw.startsWith('how') || kw.startsWith('what') || kw.startsWith('why') ? 'informational' : null
        };
      };
      return {
        primary: (parsed.primary || []).map(k => toKeywordObj(k, 'AI_FALLBACK')).filter(Boolean),
        secondary: (parsed.secondary || []).map(k => toKeywordObj(k, 'AI_FALLBACK')).filter(Boolean),
        longTail: (parsed.longTail || []).map(k => toKeywordObj(k, 'AI_FALLBACK')).filter(Boolean),
        questions: (parsed.questions || []).map(k => toKeywordObj(k, 'AI_FALLBACK')).filter(Boolean)
      };
    }
  } catch (e) {
    console.warn('[Keyword Intelligence] AI fallback failed:', e.message);
  }
  return generateFallbackKeywords(productName, companyName, industry);
}

function generateFallbackKeywords(productName, companyName, industry) {
  const product = productName || companyName || '';
  const ind = industry || 'technology';
  const terms = [];
  const productWords = product.toLowerCase().split(' ').filter(w => w.length > 2);
  if (productWords.length > 0) {
    const root = productWords.slice(0, 2).join(' ');
    terms.push(root, `${root} pricing`, `${root} features`, `${root} review`, `${root} alternatives`);
    terms.push(`best ${root}`, `${root} for business`, `how does ${root} work`, `${root} vs`);
  }
  terms.push(`${ind} software`, `best ${ind} tools`, `${ind} platform`, `${ind} solutions`);
  terms.push(`top ${ind} companies`, `${ind} trends 2026`, `${ind} automation`);
  terms.push(`how to choose ${ind} software`, `${ind} best practices`);
  const toObj = (kw) => ({
    keyword: kw, searchVolume: null, keywordDifficulty: null, cpc: null,
    competition: null, source: 'AI_FALLBACK', confidence: null,
    metricType: 'topic_idea_only', label: 'ai_generated',
    validationStatus: 'AI_GENERATED'
  });
  return {
    primary: terms.slice(0, 5).map(toObj),
    secondary: terms.slice(5, 12).map(toObj),
    longTail: terms.slice(12, 20).map(toObj),
    questions: [`what is ${product || ind}`, `how does ${product || ind} work`, `why choose ${product || ind}`, `is ${product || ind} worth it`].map(toObj)
  };
}
  ...
  try {
    ...
    if (metricsResult.success && metricsResult.data) {
      ...
      const enrichKeyword = (kw) => {
        ...
        // Mark with enterprise data quality
        return markDataQuality(
          ...
          {
            provider: 'DataForSEO',
            method: usedLabs ? 'labs_keyword_ideas' : 'search_volume',
      ...
      result.metadata.qualityScore = calculateQualityScore([
        ...result.primaryKeywords,
        ...result.secondaryKeywords,
        ...result.longTailKeywords,
        ...result.questionKeywords
 ...
 * Marked clearly as ESTIMATED (MEDIUM confidence) — the UI shows real numbers, never N/A.
 */
async function estimateMissingMetricsWithAI(result, allKeywords) {
  ...
  const uniqueMissing = [...new Set(missing.map(k => k.keyword))].slice(0, 30);
  try {
    ...
    if (result_ai.success && Array.isArray(result_ai.data?.keywords)) {
      ...
      const apply = (kw) => {
        ...
        return markDataQuality(
          ...
          {
            provider: 'AI',
            method: 'ai_metric_estimation',
            reason: 'provider_metrics_missing',
