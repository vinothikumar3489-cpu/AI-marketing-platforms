...
export async function generateKeywordIntelligence({
  ...
  try {
    ...
    if (totalAfterEnrich === 0) {
      ...
      if (aiFallback) {
        result.primaryKeywords = aiFallback.primary || [];
        result.secondaryKeywords = aiFallback.secondary || [];
        result.longTailKeywords = aiFallback.longTail || [];
        result.questionKeywords = aiFallback.questions || [];
        result.metadata = {
          ...result.metadata,
