export function mapCampaignPlanToPersistence(generatedPlan, { userId, chatId, existingPlan }) {
  ...
  const base = {
    ...
    creativeAngles: mergeField(d.creativeAngles, existingPlan?.creativeAngles),
    budget: mergeField(d.budget, existingPlan?.budget),
    forecast: mergeField(d.forecast, existingPlan?.forecast),
    roi: mergeField(d.roi, existingPlan?.roi),
