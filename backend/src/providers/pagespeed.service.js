...
export async function getPageSpeedAudit(url, strategy = 'mobile') {
  ...
  // Shared cache (same key space as the evidence module) — one PageSpeed
  // implementation, one cache, zero duplicate API calls across pipeline phases.
  // Failures are NOT cached: only successful audits seed the cache, so a
  // transient failure is genuinely re-attempted on the next call.
  const cacheKey = `pagespeed:${strategy}:${cacheKeyUrl(url)}`;

  try {
    return await memoize(cacheKey, CACHE_TTL_MS, async () => {
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        const response = await fetch(apiUrl, { signal: AbortSignal.timeout(REQUEST_TIMEOUT_MS) });
        ...
        if (response.ok) {
          ...
          return { success: false, error: 'PageSpeed API returned no lighthouse result' };
        }
    ...
    return { success: false, error: err.message || 'Max retries exceeded' };
  }
}
...
function extractSeoAudits(audits) {
  return {
    ...
    isOnHttps: audits['is-on-https']?.score === 1,
    source: 'PageSpeed'
  };
}
