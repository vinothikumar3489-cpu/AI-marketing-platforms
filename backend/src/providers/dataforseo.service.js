...
function getAuthHeader() {
  if (!DATAFORSEO_LOGIN || !DATAFORSEO_PASSWORD) {
    console.warn('ΓÜá∩╕Å DataForSEO credentials not configured');
    return null;
  }
...
async function dataforseoRequest(endpoint, method = 'POST', body = null) {
  ...
  if (!VALID_ENDPOINTS.has(endpoint)) {
    console.error(`Γ¥î [DataForSEO] Invalid endpoint: ${endpoint}`);
...
function normalizeKeywordMetrics(response) {
  ...
  if (task.status_code !== 20000 || !task.result) {
    console.warn(`ΓÜá∩╕Å [DataForSEO] Task failed: ${task.status_message}`);
    return null;
  }
...
function normalizeBacklinkSummary(response) {
  ...
  const task = response.tasks[0];
  if (task.status_code !== 20000 || !task.result) {
    console.warn(`ΓÜá∩╕Å [DataForSEO] Backlink task failed: ${task.status_message}`);
    return null;
  }
...
function normalizeDomainAuthority(response) {
  ...
  const task = response.tasks[0];
  if (task.status_code !== 20000 || !task.result) {
    console.warn(`ΓÜá∩╕Å [DataForSEO] Domain analytics task failed: ${task.status_message}`);
    return null;
  }
...
export async function getDomainData(domain) {
  ...
  try {
    ...
    console.error(`Γ¥î [DataForSEO] Domain data fetch failed:`, error.message);
    return { success: false, error: error.message };
  }
}
...
// DataForSEO health check endpoint (added for integrations controller compat)
